...
!
      ...
      MODULE EMODDATA
!  
!  MODULE STORING ELEMENT MODULUS DATA FOR A NONLINEAR PROBLEM.
!
      REAL :: EE(20000)
      END MODULE EMODDATA
...
!  INPUT AND APPLY POINT FORCE DATA.                                     
      ...
      END IF                               
!                                                                        
!  INPUT AND APPLY DISTRIBUTED FORCE DATA.                               
      IF(NBC2F > 0) THEN                                             
        Each distributed force in turn: DO IF=1,NBC2F                                                    
        READ(5,*) NBP,PX,PY                                               
        READ(5,*) (NPB(N),N=1,NBP)                                        
        NS=NBP-1                                                           
        Each boundary point in turn: DO IS=1,NS                                                       
        I1=NPB(IS)                                                         
        I2=NPB(IS+1)                                                       
        SIDE=SQRT((X(I1)-X(I2))**2+(Y(I1)-Y(I2))**2)                       
        FXM=0.5*PX*SIDE                                                    
        FX(I1)=FX(I1)+FXM                                                  
        FX(I2)=FX(I2)+FXM                                                  
        FYM=0.5*PY*SIDE                                                    
        ...
        END DO Each distributed force in turn
      END IF                                                  
!                                                                        
...
!  APPLIED TO THE BOUNDARY SEGMENT.
      ...
      SUBROUTINE SOLVE2                                            
!                                                                        
!  SUBPROGRAM FOR SOLVING BY GAUSS-SEIDEL METHOD THE EQUATIONS    
!  OBTAINED FROM THE FINITE ELEMENT FORMULATION OF BIHARMONIC PROBLEMS.  
!
!  THIS VERSION ALLOWS FOR NONLINEAR MATERIAL PROPERTIES TO BE UPDATED 
!  PERIODICALLY.
!
      USE EQNSDATA
      USE MESHDATA
...
!  TEST FOR CONVERGENCE.                                                 
      ...
      IF(ERROR < TOLER) EXIT                                         
!                                                                        
!  OUTPUT PROGRESS INFORMATION AND UPDATE MATERIAL PROPERTIES
!  EVERY IFREQ CYCLES, UNLESS IFREQ=0.       
      IF(IFREQ /= 0) THEN                                             
        IF(MOD(ITER,IFREQ) == 0) THEN 
          WRITE(*,63) ITER,ERROR                    
 63       FORMAT(I8,E15.4)
          CALL NONLIN
        END IF
...
!  OUTPUT NUMBER OF ITERATIONS AND TOLERANCE FOR CONVERGED SOLUTION.     
 ...
 65   FORMAT(/"ITERATION CONVERGED TO A TOLERANCE OF",E12.4,            
     &        "     AFTER",I8," CYCLES")                                     
      ...
      SUBROUTINE NONLIN
!                                                                        
!  SUBPROGRAM FOR UPDATING MATERIAL PROPERTIES FOR A NONLINEAR 
!  PROBLEM DURING THE ITERATIVE SOLUTION OF THE EQUATIONS.                                           
!
      USE MESHDATA
      USE EQNSDATA
      USE MATLDATA
      ...
      USE EMODDATA
!
      REAL :: B(3,6),D(3,3),BTD(6,3),ESTIFF(6,6)
      INTEGER :: IJK(3),IFLAG=0
!
!  INITIALISE ELEMENT VALUES OF MODULUS, AND CHECK FOR TEMPERATURE 
!  CHANGES.
      IF(IFLAG == 0) THEN
        SUMDELT=0.
        Each element in turn: DO M=1,NEL
        MAT=MATM(M)
        EE(M)=E(MAT)
        SUMDELT=SUMDELT+ABS(DELTAT(M))
        END DO Each element in turn
        WRITE(6,61)
 61     FORMAT(/"NONLINEAR MODULUS")
        IF(SUMDELT > 0.) THEN
          WRITE(6,62)
 62       FORMAT(/"PROGRAM NOT VALID FOR TEMPERATURE CHANGES")
          STOP
        END IF
        IFLAG=1
      END IF                                                                                  
!                                                                        
!  SET INITIAL VALUES OF STIFFNESSES.      
      Each overall row in turn: DO IROW=1,NNP                                                    
      Each overall column in turn: DO IC=1,13                                                        
      OKXX(IROW,IC)=0.                                                   
      OKXY(IROW,IC)=0.                                                   
...
!  COMPUTE THE SELF-FLEXIBILITY SUBMATRICES.                             
      ...
      END DO Each node in turn                                            
!                                                                        
!  APPLY THE BOUNDARY RESTRAINT CONDITIONS.                                        
      Each restrained node in turn: DO N=1,NBC3P                                                    
      I=NPB(N)                                                           
!                                                                        
!  NODAL POINT DISPLACEMENTS PRESCRIBED.                                 
...
!  NODE RESTRAINED TO MOVE IN Y-DIRECTION ONLY.
      ...
      FUNCTION ENONLIN(MAT,EXX,EYY,EXY,SIGXX,SIGYY,SIGXY)
!
!  SUBPROGRAM TO FIND THE MODULUS OF A NONLINEAR MATERIAL FROM
!  THE LOCAL STATE OF STRESS OR STRAIN.
!  THIS VERSION FOR PLANE STRESS ONLY.
!
      USE MATLDATA
      USE EMODDATA
!
      REAL, PARAMETER :: E0=50.E9, SIG0=10.E6, N=1.4
!
      IF(MAT > 1) THEN 
        ENONLIN=E(MAT)
        RETURN
      END IF
!
!  EQUIVALENT STRESS IN SIMPLE TENSION, ASSUMING PLANE STRESS.
      SIGE=SQRT(SIGXX**2+SIGYY**2-SIGXX*SIGYY+3.*SIGXY**2)
      IF(SIGE/SIG0 < 1.E-3) SIGE=SIG0*1.E-3      
!
!  NONLINEAR MODULUS WITH POWER-LAW DEPENDENCE ON STRESS.
      ENONLIN=E0*(SIGE/SIG0)**(1.-N)
      END FUNCTION ENONLIN



      SUBROUTINE  OUTPUT                                                 
!                                                                        
!  SUBPROGRAM TO OUTPUT THE FINAL RESULTS FOR A NONLINEAR 
!  MATERIAL PROBLEM.                               
!
      USE MESHDATA
      USE EQNSDATA
...
!  COMPUTE AND OUTPUT THE ELEMENT STRAIN AND STRESS COMPONENTS.          
 ...
 63   FORMAT(/"    M     EXX         EYY         EXY         ET",         
     ...
     &        +BK(M)*V(K))/AREA(M)                                       
      ...
      SIGXY=FACT*0.5*(1.-NU(MAT))*EXY
      SIGE=SQRT(SIGXX**2+SIGYY**2-SIGXX*SIGYY+3.*SIGXY**2)                                 
      WRITE(6,64) M,EXX,EYY,EXY,ET,SIGXX,SIGYY,SIGXY,SIGE,EE(M)
 64   FORMAT(I6,9E12.4)
      END DO Each element in turn                                                           
      RETURN                                                             
